...
| --- | ---: | --- |
| `ollamaCloudCopilot.maxOutputTokens` | `32768` | Requested generation ceiling, capped by the selected model |
| `ollamaCloudCopilot.requestTimeoutSeconds` | `600` | Inference timeout in seconds |
...
Thinking-capable models expose a native control in the model picker. GPT-OSS offers Low, Medium, and High. MiniMax models offer Low through Max because the hosted backend keeps their reasoning enabled. Other thinking models can offer Off, Low, Medium, High, and Max. The picker selection applies to that request through Ollama's native `think` field.
...
- **A request times out:** increase `ollamaCloudCopilot.requestTimeoutSeconds`; agentic requests can run longer than ordinary chat.
